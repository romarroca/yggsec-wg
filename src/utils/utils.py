...
def _resolve(argv: Iterable[str]) -> List[str]:
    a = list(argv)
    if not a: raise ValueError("empty argv")
    exe = a[0]
...
def ensure_dir(p: str | Path, mode: int = 0o700) -> None:
    Path(p).mkdir(parents=True, exist_ok=True); os.chmod(p, mode)
...
def write_text_atomic(p: str | Path, data: str, mode: int = 0o600) -> None:
    ...
    with tempfile.NamedTemporaryFile("w", dir=str(p.parent), delete=False, 
                                   prefix=f".{p.name}.tmp.") as tmp:
        tmp.write(data); tmp.flush(); os.fsync(tmp.fileno())
        os.fchmod(tmp.fileno(), mode); name = tmp.name
    os.replace(name, p)
