...
def validate_nft_json_structure(nft_data):
    ...
    for item in nftables_list:
        ...
        if "table" in item:
            ...
            if (isinstance(table_def, dict) and 
                table_def.get("family") == "inet" and
                table_def.get("name") == NFT_TABLE):
...
def test_json_config_parsing():
    ...
    try:
        ...
        if generated_config:
            ...
            print("-" * 40)
            print(generated_config[:500] + "..." if len(generated_config) > 500 else generated_config)
            print("-" * 40)
        else:
...
def persist_vpnfw_table():
    ...
    try:
        with tempfile.NamedTemporaryFile(mode='w', dir='/etc/nftables.d', 
                                       prefix='.vpnfw.nft.tmp.', delete=False) as tmp_file:
...
def reset_firewall(policy="drop"):
    ...
    if policy == "drop":
        run_cmd(["nft", "add", "rule", "inet", NFT_TABLE, NFT_CHAIN,
...
def add_rule(src_ip=None, dst_ip=None, dst_port=None,
             proto=None, action="TRUST",
             position="append", index=None, name=None):
    """
    ...
    if not validate_rule_name(name):
        return False, "Rule name is required and must contain only alphanumeric characters, spaces, dashes, underscores (max 50 chars)"
    ...
    def _build_and_run(one_proto: str | None, suffix_name: str = ""):
        ...
        if one_proto and dst_port:
            ...
            # ANY protocol
            if dst_port:
                return False, "Port filter requires TCP or UDP. Choose TCP, UDP, or TCP+UDP."
...
def list_rules_table():
    ...
    for line in output.splitlines():
        line = line.strip()
        if not line or line.startswith(("chain", "table", "type filter", "ct state")) or line in ("{", "}"):
            continue
...
def delete_rule(handle):
    ...
    ok, msg = run_cmd([
        "nft", "delete", "rule", "inet", NFT_TABLE, NFT_CHAIN,
