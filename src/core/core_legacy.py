...
def regenerate_hub_keys():
    ...
    try:
        ...
        os.replace(
            os.path.join(config.keys_dir, "hub-new_private.key"),
            os.path.join(config.keys_dir, "hub_private.key"),
        )
        os.replace(
            os.path.join(config.keys_dir, "hub-new_public.key"),
...
def regenerate_spoke_keys(name):
    ...
    try:
        ...
        os.replace(
            os.path.join(config.keys_dir, f"{name}-new_private.key"),
            os.path.join(config.keys_dir, f"{name}_private.key"),
        )
        os.replace(
            os.path.join(config.keys_dir, f"{name}-new_public.key"),
            os.path.join(config.keys_dir, f"{name}_public.key"),
        )
